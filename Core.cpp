...
void Core::Initialize( )
{
	...
	// Set window icon
	SDL_SetWindowIcon(m_pWindow, IMG_Load("Resources/Logo.png"));
...
void Core::Run( )
{
	...
	Engine* pEngine{ new Engine{m_Window} };
	...
	while ( !quit )
	{
		...
		while ( SDL_PollEvent( &e ) != 0 )
		{
			...
			case SDL_QUIT:
				...
				break;
			case SDL_KEYDOWN:
				pEngine->ProcessKeyDownEvent(e.key);
				break;
			case SDL_KEYUP:
				pEngine->ProcessKeyUpEvent(e.key);
				break;
			case SDL_MOUSEMOTION:
				e.motion.y = int(m_Window.height) - e.motion.y;
				pEngine->ProcessMouseMotionEvent(e.motion);
				break;
			case SDL_MOUSEBUTTONDOWN:
				e.button.y = int(m_Window.height) - e.button.y;
				pEngine->ProcessMouseDownEvent(e.button);
				break;
			case SDL_MOUSEBUTTONUP:
				e.button.y = int(m_Window.height) - e.button.y;
				pEngine->ProcessMouseUpEvent(e.button);
				break;
			}
		}
